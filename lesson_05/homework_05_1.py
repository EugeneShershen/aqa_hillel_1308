...
def search_with_criteria(dict_car_data, criteria):
    print('-' * 100)

    sorted_car_data_items = sorted(dict_car_data.items(), key=lambda x: x[1][4])

    search_car_data = {key: value for key, value in sorted_car_data_items if value[1] >= criteria[0]
                       if value[2] >= criteria[1] if value[4] <= criteria[2]}

    count = 1
    for key, value in search_car_data.items():
        if count > 5:
            break
        print(key, value)
        count += 1
...
search_with_criteria(car_data, search_criteria)
